...
import com.virtuslab.ideprobe.jsonrpc.RemoteException
...
import com.virtuslab.ideprobe.protocol.TestStatus
...
final class ProbeDriverTest extends IntegrationTestSuite with Assertions {
  ...
  def collectErrors(): Unit = fixture.run { intelliJ =>
    intelliJ.probe.invokeActionAsync("com.virtuslab.ideprobe.test.ThrowingAction")
  ...
  def backgroundTask(): Unit = fixture.run { intelliJ =>
    ...
    assertDuration(min = 15.seconds, max = 5.minutes) {
      intelliJ.probe.invokeAction("com.virtuslab.ideprobe.test.BackgroundTaskAction15s")
    }
  ...
  def freezeInspector(): Unit =
    fixture
      ...
      .withDisplay
      .run { intelliJ =>
        intelliJ.probe.invokeAction("com.virtuslab.ideprobe.test.FreezingAction")
  ...
  def runTests(): Unit = {
    buildTestFixture
      .run { intelliJ =>
        ...
        assertEquals(TestStatus.Passed, testA.status)
  ...
  def failsOnUnexpectedWindow(): Unit = fixture.withDisplay.run { intellij =>
    val action = "com.virtuslab.ideprobe.test.OpenWindowAction"
