...
class FileDownloader(private val baseDirectory: Path) {
  ...
  def download(url: URL, optional: Boolean = false): Path = try {
    ...
    if (targetFile.toFile.exists()) {
      println(s"$targetFile already exists, recovering failed install...")
    ...
    case e: Exception if optional =>
      println(s"Can't download${if (optional) " optional" else ""} $url: $e")
      Paths.get("")
  }
  ...
  private def downloadNative(url: URL)(progressCallback: ProgressCallback): Path = {
    ...
    if (remoteMetaData.length == localLength) {
      println(s"Part file already downloaded: recovering interrupted install...")
      return to
    }

    if (to.toFile.exists() && isResumeSupported(url)) {
      connection.setRequestProperty("Range", s"bytes=$localLength-")
      println(s"Resuming download of $url to $to")
    } else {
      Files.deleteIfExists(to)
      println(s"Starting download $url to $to")
    }
...
object FileDownloader {
  ...
  def apply(baseDirectory: Path): FileDownloader = new FileDownloader(baseDirectory.resolve("cached"))
