...
class IdeProbeConfigTest extends IdeProbeFixture {
  ...
  def loadsDefaultValuesFromReferenceConfFile(): Unit = {
    ...
    assertEquals("222.4459.24", probeConfig.intellij.asInstanceOf[IntellijConfig.Default].version.build)
    assertEquals(Some("2022.2.4"), probeConfig.intellij.asInstanceOf[IntellijConfig.Default].version.release)
