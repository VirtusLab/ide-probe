...
class IdeProbeConfigTest extends IdeProbeFixture {
  ...
  def loadsDefaultValuesFromReferenceConfFile(): Unit = {
    ...
    assertEquals("223.7571.182", probeConfig.intellij.asInstanceOf[IntellijConfig.Default].version.build)
    assertEquals(Some("2022.3"), probeConfig.intellij.asInstanceOf[IntellijConfig.Default].version.release)
